...
| 字段名 | 类型 | 必填 | 说明 | 默认值 |
|--------|------|------|------|--------|
| soundEnabled | boolean | 否 | 音效开关（true=开启，false=关闭） | true |
| musicEnabled | boolean | 否 | 背景音乐开关（true=开启，false=关闭） | true |
...
**settings 字段说明：**
- `settings` 字段用于存储用户的各种设置项，采用JSON对象格式，便于后续扩展
- 当前包含音效和背景音乐开关，后续可以在此字段中添加更多设置项
- 如果客户端未传递 `settings` 字段，服务器应使用默认值（所有设置项为true）
- 如果 `settings` 中缺少某个设置项，服务器应使用该设置项的默认值
...
CREATE TABLE `user_game_data` (
  ...
  `settings` JSON DEFAULT NULL COMMENT '用户设置（JSON格式，包含音效、背景音乐等设置项）',
...
### v1.4.0 (2024-01-XX)
- **新增字段**：`settings` - 用户设置对象（JSON格式）
- **新增设置项**：`settings.soundEnabled` - 音效开关（boolean类型，默认true）
- **新增设置项**：`settings.musicEnabled` - 背景音乐开关（boolean类型，默认true）
- **数据库变更**：`user_game_data` 表新增 `settings` 字段（JSON类型）
- **接口更新**：获取和保存用户游戏数据接口支持 `settings` 字段
- **说明**：
  - `settings` 字段采用JSON对象格式，便于后续扩展新的设置项
  - `settings.soundEnabled` 控制游戏内所有点击音效的播放（true=开启，false=关闭）
  - `settings.musicEnabled` 控制背景音乐的播放（true=开启，false=关闭）
  - `settings` 为可选字段，如果客户端未传递，服务器应使用默认值（所有设置项为true）
  - 如果 `settings` 中缺少某个设置项，服务器应使用该设置项的默认值
  - 客户端在设置窗口中修改开关后，会通过定时保存自动同步到服务器
